...
async def get_embedding(text: str, is_query: bool = False) -> List[float]:
    ...
    if key in _cache:
        vec, ts = _cache[key]
        if now - ts < settings.embed_cache_ttl: return vec
    async with httpx.AsyncClient(timeout=60) as client:
        r = await client.post(f"{settings.ollama_base_url}/api/embeddings",
            json={"model": settings.ollama_embed_model, "prompt": key})
        r.raise_for_status()
        vec = r.json()["embedding"]
    _cache[key] = (vec, now)
    for k in [k for k, (_, t) in list(_cache.items()) if now - t > settings.embed_cache_ttl]:
        del _cache[k]
    return vec
