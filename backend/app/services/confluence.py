...
async def ingest_confluence(spaces: Optional[List[str]] = None):
    ...
    if not targets:
        _status = {"running": False, "progress": 0, "total": 0, "message": "No spaces configured"}
        return
        ...
        for space in targets:
            ...
            while True:
                resp = await c.get(f"{settings.confluence_url}/wiki/rest/api/content",
                    params={"spaceKey": space, "type": "page", "start": start,
                            "limit": 50, "expand": "body.storage,version,space"})
    ...
    async with aiosqlite.connect(settings.sqlite_path) as db:
        for i, (page, space) in enumerate(all_pages):
            ...
            await db.execute(
                "INSERT OR REPLACE INTO page_index (page_id,space_key,title,url,last_modified,indexed_at) VALUES (?,?,?,?,?,?)",
            ...
            if len(points) >= settings.upsert_batch_size:
                await upsert_points(points); points = []
        await db.commit()
    if points: await upsert_points(points)
    await invalidate_cache()
    _status = {"running": False, "progress": _status["total"],
               "total": _status["total"], "message": "Complete"}
