...
async def cache_lookup(question: str) -> Optional[dict]:
    ...
    vec = await get_embedding(question, is_query=True)
    ...
    if time.time() - payload.get("cached_at", 0) > settings.semantic_cache_ttl: return None
    return payload
    ...
    await client.upsert(
        ...
        points=[models.PointStruct(id=str(uuid.uuid4()), vector=vec,
            payload={"question": question, "answer": answer,
                     "sources": sources, "cached_at": time.time()})])
    await client.close()
